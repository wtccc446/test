...
void Relay_Ctr_Cmd(uint8_t *buff)
{
	...
	ret=Relay_ExpendIoOP(opbuff_ch,opbuff_value,num);
	if (ret!=0)
		MessageSend("Relay OUTPUT ERROR\r\n");
	else
...
void Relay_Reset_Cmd(uint8_t *buff)
{
	...
	if (buff[11]=='(' && buff[16]==')')
	{
		ic_place=StrToHEX(buff[14],buff[15]);
		if (Relay_Reset(ic_place)!=0)
		{
			MessageSend("Relay Reset Fail\r\n");
			return ;
		}
		else
		...
		MessageSend("Relay Reset Format Error\r\n");
}
